...
#define NEW_AUX_ENT(id, val) do {               \
    ...
    //fprintf(stderr, "AT_PHDR 0x%lx + 0x%lx\n",
    //            (unsigned long)info->load_addr, (unsigned long)exec->e_phoff);
                             ...
                             abi_ulong loaddr, abi_ulong hiaddr)
{
    ...
    //fprintf(stderr, "probe_guest_base 0x%lx 0x%lx\n", (unsigned long) have_guest_base, (unsigned long) reserved_va);
    ...
    if (!have_guest_base && !reserved_va) {
        ...
        //fprintf(stderr, "probe_guest_base guest_base 0x%lx 0x%lx 0x%lx\n", (unsigned long) guest_base,
        //    (unsigned long) real_start, (unsigned long) loaddr);
                           ...
                           char bprm_buf[BPRM_BUF_SIZE])
{
    ...
    for (i = 0; i < ehdr->e_phnum; ++i) {
        if (phdr[i].p_type == PT_LOAD) {
            ...
            //fprintf(stderr, "pt load %d vaddr 0x%lx off 0x%lx\n",
            //        i, (unsigned long)phdr[i].p_vaddr, (unsigned long) phdr[i].p_offset);
           
            abi_ulong a = phdr[i].p_vaddr ; // - phdr[i].p_offset; /*offset is the offset in the file*/
            if (a < loaddr) {
                loaddr = a;
            }
    ...
    for (i = 0; i < ehdr->e_phnum; i++) {
        ...
        if (eppnt->p_type == PT_LOAD) {
            ...
            /*fprintf(stderr, "target_mmap 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx\n", (unsigned long)vaddr,
                        (unsigned long)vaddr_ps, (unsigned long) load_bias,
                    (unsigned long)eppnt->p_filesz + vaddr_po,(unsigned long)eppnt->p_offset - vaddr_po
            );*/
